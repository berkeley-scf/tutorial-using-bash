...
::: {.callout-important title="Different types of regular expressions"}
...
Parentheses are used with a pipe (`|`) when working with
multi-character sequences, such as `(http|ftp)` in order to indicate grouping of characters.
Also, here we need double
quotes or the shell tries to interpret the `(` as part of the regular
expression and not shell syntax.

Next let's see the use of repetition to look for more complicated multi-character patterns. For
...
Note that again parentheses were used for grouping (in "(1-)?").
...
Many tools that make use of regular expressions also allow the use of "capture groups" to store the "captured" string that matches a pattern and be able to manipulate it as a variable for subsequent output.

This can be used in `sed` as well as in string processing in Python.

We saw the use of parentheses for grouping above. Indicating groups with parentheses is also used when we need to reference back to a detected pattern when doing a replacement. This is why they are sometimes referred to as "capturing groups". For example, here we’ll find any numbers and add asterisks before and after them, with `\1` referring to the captured string.

```bash
$ text="Here's my number: 919-543-3300. They bought 731 bananas. Please call 919.554.3800."
$ echo ${text} | sed -E "s/([0-9]+)/***\1***/g"
...
```bash
$ text="Here's my number: 919-543-3300. They bought 731 bananas. Please call 919.554.3800."
$ echo ${text} | sed -E "s/([0-9]{3})[-\.]([0-9]{3})[-\.]([0-9]{4})/area code \1, number \2-\3/g"
Here's my number: area code 919, number 543-3300. They bought 731 bananas. Please call area code 919, number 554-3800.
```

Capturing groups are less common with `grep`, which finds patterns rather than replaces them, but you can used capturing groups to look for repeated matches.

For example, suppose we wanted to find repeated integer numbers:

```bash
$ text="The number 123 is repeated: 123 again"
$ echo ${text} | grep -o -E '([0-9]+).*\1'
123 is repeated: 123
$ text="The number 45 is not repeated"
$ echo ${text} | grep -o -E '([0-9]+).*\1'
```